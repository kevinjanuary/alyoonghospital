...
<?php echo form_open(base_url() . 'admin/add_bed/create' , array('class' => 'form-horizontal form-groups-bordered validate', 'enctype' => 'multipart/form-data'));?>
								
								<div class="form-group">
                                    <label class="col-md-12"><?php echo get_phrase('bed_type');?>*</label>
                                    <div class="col-md-12">
									<input type="text" class="form-control" name="bed_type"/ required placeholder= "e.g emergency bed, labour bed, operation bed ...">
									
									</div>
									</div>
									
									<div class="form-group">
                                    <label class="col-sm-12"><?php echo get_phrase('bed_ward');?>*</label>
                                    <div class="col-sm-12">
                                       <select class=" form-control" name="bed_ward_id" data-style="btn-primary btn-outline" required>
                                         <option data-tokens=""><?php echo get_phrase('select_bed_ward');?></option>
                                    	<?php 
										$departments = $this->db->get('bed_ward')->result_array();
										foreach($departments as $row):
										?>
                                    <option value="<?php echo $row['bed_ward_id'];?>"><?php echo $row['name'];?></option>
									 <?php endforeach;?>
                                    </select>
                                    </div>
                                </div>
